...
RCT_EXPORT_METHOD(checkNetworkState:(RCTResponseSenderBlock)callback)
{
  NetworkStatus status = [reach currentReachabilityStatus];
  BOOL isConnected = (status == ReachableViaWiFi || status == ReachableViaWWAN);
  NSString *statusString = isConnected ? [NSString stringWithFormat: @"connected"]: [NSString stringWithFormat:@"notConnect"];
